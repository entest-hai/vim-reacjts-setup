...
viw
```
combine with number to move multile characters, words, line  
```
<h, i, j, k> 
```
...
move word by word type 
```
w 
```
move end of word type 
```
e 
```
move by 5 words type 
```
5w 
```
delete a character type 
```
x 
```
delete two characters type 
```
2x 
```
delete a word type 
...
<r> <new>
``` 
delete a word and into insert
```
<c> <e>
``` 
delete a line and into isnert mode 
```
<c> <$> 
``` 
**macro**
```
qa to start record macro@a
do a macro
q to quit
10@a to execute
```
...
<S-s> enter surroud plugin 
<em> enter what to surround 
```
update surround 
```
<S-v> enter visual mode 
<S-s> enter surround plugin
<Flex then enter to update without change properties 
<Flex> then etner to update with removing properties 
```
